...
namespace ActionGame.ECS
{
    ...
    {
        public const uint MAX_ENTITIES = 5_000_000;
        ...
        public uint CreateEntityIndex()
        {
            ...
            entityCount++;
            if (recycleIndicies.Count > 0)
                return recycleIndicies.Pop();
            entityIndex++;
            return entityIndex - 1;
        }

        public void FreeEntityIndex(uint entity)
        {
            recycleIndicies.Push(entity);
            entityCount--;
        }
    }
}
